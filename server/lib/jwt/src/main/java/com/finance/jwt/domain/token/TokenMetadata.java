...
public enum TokenMetadata {
  ETERNAL("Authorization", 1000L * 60 * 60 * 24 * 365 * 100),
  ACCESS("Authorization", 1000L * 60 * 15),
  REFRESH("Refresh", 1000L * 60 * 60 * 24 * 15);
  ...
  public Key getSecretKey() {
    if (secretKey == null) {
      throw new IllegalStateException("SecretKey by " + header + " is not specified");
    }
    return secretKey;
  }
